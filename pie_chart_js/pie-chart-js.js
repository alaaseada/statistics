...
class PChart extends HTMLElement {
  ...
  getAngle = function (x_coord, y_coord, x_origin, y_origin) {
    var radius = Math.sqrt(
      Math.pow(Math.abs(x_coord - x_origin), 2) +
        Math.pow(Math.abs(y_coord - y_origin), 2)
    );
    if (
      Math.acos(Math.abs(x_coord - x_origin) / radius).toFixed(6) ==
        Math.asin(Math.abs(y_coord - y_origin) / radius).toFixed(6) &&
      radius <= this.radius
    ) {
  ...
  drawChart = function () {
    ...
    for (var i = 0; i < this.names.length; i++) {
      ...
      var shorten = 0;
      if (
        ctx.measureText(this.names[i]).width >
        parseInt(c.width * 0.12019231) - 15
      ) {
        while (
          ctx.measureText(
            this.shorten_text(this.names[i], this.names[i].length - shorten)
          ).width >
            parseInt(c.width * 0.12019231) - 15 &&
          this.names[i].length - shorten > 0
        ) {
          shorten++;
        }
      }
    ...
    for (var i = 0; i < share_percentage.length; i++) {
      ...
      var arc_start = this.getPoint(
        this.x_origin,
        this.y_origin,
        this.radius,
        start * 2 * Math.PI
      );
      var arc_end = this.getPoint(
        this.x_origin,
        this.y_origin,
        this.radius,
        end * 2 * Math.PI
      ...
      for (var i = 0; i < this.arcs.length; i++) {
        if (mouse_angle > this.arcs[i][0] && mouse_angle < this.arcs[i][1]) {
          if (!this.shadowRoot.querySelector('#Mouse_Point_Div')) {
            ...
            div.innerHTML =
              'Title: ' + this.names[i] + '<br>' + 'Value: ' + this.vals[i];
